...
class Worker(QThread):
    def __init__(self, parent = None,testconf=None):
    ...
    def run(self):
        ...
        telnet_flag = self.do_test(self.myconf,self.testconf)

        if telnet_flag==0:
            #开始测试z
            self.do_protest(self.myconf,self.testconf)
        elif telnet_flag == 1:
    ...
    ##AP校准测试及准备##
    def do_test(self,myconf,testconf):
        try:
            # testconf = TestConf()
            # myconf = MyConf()
    ...
    def telnet_default_ip_and_do_something(self):
        ...
        try:
            ...
            tn = telnetlib.Telnet(telnet_ip)
            login_str = tn.read_until("login: ",10)
            tn.write(username+'\n')
    ...
    def do_protest(self,myconf,testconf):
        global test_mode
        global test_result_fail
        ...
        how_to_do_test = testconf.get_howtotest()
        # test_result_fail = 0
        jiaozhun_result = 0
        pmgr_link = myconf.get_pmgr_link()
        # ---------------------------------
        ...
        if(test_mode == 2):
            ...
            if(testconf.get_is_or_not_jiaozhun() == '1'):
                if testconf.get_use_art2() == '1':
                    # 更改art执行类型
                    self.change_art_execute_type(0)

                    art_jiaozhun.only_2g_jiaozhun()
            ...
            print os.getcwd()
            print "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
            self.change_art_execute_type(1)
            # 在执行rx前，先执行cart获取2g的所有统计项
            rx_2g_dictionary = self.read_rx_out()
            # # 开启子进程执行cart
            # old_path = os.getcwd()
            # os.chdir(old_path + '\\ART2\\art2_ver_4_9_844_release\\bin')
            # popen = subprocess.Popen(["cart.exe"],stdout=subprocess.PIPE)
            ...
            #             aout = special_a.replace('----------------------------------------------------------------------------------',popen.stdout.readline())
            #             # aout=aout.replace("\r","")
            #         if aout.startswith("4003 INFO"):
            #             self.printstr(aout+"\n", QColor(0, 0, 0, 110))
            #             m = aout.replace('\n', '').replace('\r','').replace('4003 INFO', '').split(' ')
            #             while '' in m:
            #                 m.remove('')
            #             rx_2g_dictionary[m[0] + '_' + m[4].lower() + '_' + m[6]] = m[1]
            #         elif "" == aout:
            #             break
            #     except Exception as e:
            #         print e
            # os.chdir(old_path)
            # print rx_2g_dictionary
            print "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
            ...
            if testconf.get_run_2g_rx() == '1':
                self.loop_rx(rx_freq, rx_rate, rx_chain, pre_line,rx_2g_dictionary)
            if fail_stop_flag == '1':
                if test_result_fail != 0:
                    self.printstr("\n\n\nfail\n",QColor(0, 0, 0, 110))
                    return
            ...
            # 在执行rx前，先执行cart获取2g的所有统计项
            print "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
            self.change_art_execute_type(2)
            rx_5g_dictionary = self.read_rx_out()
            # old_path = os.getcwd()
            # os.chdir(old_path + '\\ART2\\art2_ver_4_9_844_release\\bin')
            # # self.printstr('\n  ----use ART2 begin-----\n', QColor(0, 0, 0, 110))
            # # 开启子进程执行cart
            ...
            #             while '' in m:
            #                 m.remove('')
            #             rx_5g_dictionary[m[0] + '_' + m[4] + '_' + m[6]] = m[1]
            #             # self.printstr(">>>>>>>>>>>>>>>>>>>>>>>",QColor(0, 0, 0, 110))
            #         elif "" == aout:
            #             break
            #     except Exception as e:
            #         print "error"
            ...
            if testconf.get_run_5g_rx() == '1':
                self.loop_rx(rx_freq, rx_rate, rx_chain,pre_line,rx_5g_dictionary)
            #self.after_rx()
            if fail_stop_flag == '1':
                if test_result_fail != 0:
                    return
    ...
    def loop_tx(self, freq_all, rate_all, chain_all, setting_limit, pre_line=None):
        ...
        data_base = DataBaseOP()
        #读取当前测试sn
        test_sn = myconf.get_sn_num()

        #初始化日志实例，并赋值serialno
        rtest = RFtest(test_sn)
            ...
            for key, freq in freq_all[i].items():
                ...
                for rate in rate_this:
                    ...
                    for chain in chain_this:
                        ...
                        ###设置日志属性###
                        rtest.setFrequency(freq)
                        rtest.setRate(rate)
                        rtest.setChannelNo(chain)
                        ###设置日志属性###
                        ...
                        ###设置retest log 属性###
                        rtest.setSpectrum(mymask)
                        rtest.setStandardRF(standard_str)
                        ###设置retest log 属性###
                        ...
                        ###设置retest log 属性###
                        rtest.setPower(meas_pwr)
                        rtest.setPowerDelta(delta)
                        if power_result==1:                                 #结果
                            rtest.setIsPowerPass(0)
                        else:
                            rtest.setIsPowerPass(1)
                        ###设置retest log 属性###
                        ...
                        if d_type == "80211n" or d_type == "80211ac" :
                            ...
                            ###设置retest log 属性###
                            rtest.setEvm(round(evmAvgAll_0,2))
                            rtest.setTargetEvm(stand_evm)
                            if evm_result==1:                                              #结果
                                rtest.setIsEvmPass(0)
                            else:
                                rtest.setIsEvmPass(1)
                            ###设置retest log 属性###
                        ...
                        if d_type == "80211n" or d_type == "80211ac":
                            ...
                            ###设置retest log 属性###
                            rtest.setFreq(round(freqErrorHz_0/string.atoi(freq),2))
                            rtest.setTargetFreq(txppm_limit)
                            if evm_ppm_result==1:                                                               #结果
                                rtest.setIsFreqPass(0)
                            else:
                                rtest.setIsFreqPass(1)
                            ###设置retest log 属性###
                            ...
                            ###设置retest log 属性###
                            rtest.setFreq(round(freqErr_0/string.atoi(freq),2))
                            rtest.setTargetFreq(txppm_limit)
                            if evm_ppm_result==1:                                                               #结果
                                rtest.setIsFreqPass(0)
                            else:
                        ...
                        evmDelta = float(rtest.getTargetEvm())-float(rtest.getEvm())
                        rtest.setEvmDelta(evmDelta)
                        freqDelta = float(rtest.getTargetFreq())-float(rtest.getFreq())
                        rtest.setFreqDelta(freqDelta)
                        # zdRtest = rtest.__dict__
                        # jsonStr = json.dumps(zdRtest)
                        # ##保存数据库##
                        # print 'Json String'
                        # print jsonStr
                        # try:
    ...
    def create_command_rx_5g(self, freq, rate, chain, stat):
        ...
        return cmd_rx
    # 备选方案，修改了参数
    def loop_rx(self, freq_all, rate_all, chain_all, pre_line=None,rx_dictionary={}):
        global dll
        global test_result_fail
            ...
            for key ,freq in freq_all[i].items():
                ...
                for rate in rate_this:
                    for chain in chain_this:
                        ...
                        # print correct
                        # 备选方案代码
                        print freq + "_" + rate + "_" + chain+'\r'
                        correct = rx_dictionary.get(freq + "_" + rate + "_" + chain)
                        print 'invoking loop_rx method '
                        # correct = '80'
                        print correct
                        if correct == None:
                            #continue
    ...
    def loop_loop_rx(self, freq, rate, chain):
        ...
        print 'invoking loop_loop_rx method'
    ...
    #change art execute type 0 校准 1 2grx 2 5grx
    def change_art_execute_type(self,type=0):
        flow_file = os.getcwd()+"\\ART2\\art2_ver_4_9_844_release\\command\\test_flow.art"
        f = open(flow_file, "r")
        line_list = f.readlines()
        f.close()
        for line in line_list:
            if "assign executeType=" in line:
                index = line_list.index(line)
                line_list[index] = "assign executeType=" + str(type) + "\n"
                break
        f2w = open(flow_file, "w+")
        f2w.writelines(line_list)
        f2w.close()

    def read_rx_out(self):
        rx_dictionary = {}
        old_path = os.getcwd()
        os.chdir(old_path + '\\ART2\\art2_ver_4_9_844_release\\bin')
        # self.printstr('\n  ----use ART2 begin-----\n', QColor(0, 0, 0, 110))
        ...
        while True:
            try:
                ...
                if aout.startswith("4003 INFO"):
                    self.printstr(aout + "\n", QColor(0, 0, 0, 110))
                    m = aout.replace('4003 INFO', '').split()
                    rx_dictionary[m[0] + '_' + m[4].lower() + '_' + m[6]] = m[1]
                    # self.printstr(">>>>>>>>>>>>>>>>>>>>>>>",QColor(0, 0, 0, 110))
                elif "" == aout:
                    break
            except Exception as e:
        ...
        os.chdir(old_path)
        print rx_dictionary
        return rx_dictionary


class Worker_WE2622(Worker):
    def __init__(self, parent = None,testconf=None):
        Worker.__init__(self, parent = None,testconf=testconf)

class Worker_WE2622_prep(Worker):
    def __init__(self, parent = None,testconf=None):
        Worker.__init__(self, parent = None,testconf=testconf)

class Worker_I22B(Worker):
    def __init__(self, parent = None,testconf=None):
        Worker.__init__(self, parent=None, testconf=testconf)

class Worker_I23B(Worker):
    def __init__(self, parent=None, testconf=None):
        Worker.__init__(self, parent=None, testconf=testconf)

class Worker_G22B(Worker):
    def __init__(self, parent = None,testconf=None):
        Worker.__init__(self, parent=None, testconf=testconf)

class Worker_G23B(Worker):
    def __init__(self, parent = None,testconf=None):
        Worker.__init__(self, parent=None, testconf=testconf)
