 ...
 *				- =0: Must set 0 for IQxel/IQxel80
 *				- =1: Single IQxel160 tester 2x2 80Mhz MIMO mode;if ipAddress1 and ipAddress2 both must be the same (IQxel160's IP address)
 ...
 *\return 0 if successful; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_Async_RequestTX(unsigned int freq, LP_Async_BWType bwType, double powerDbm, int VSAGPort);
 ...
 *\return 0 if successful; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_Async_RequestRX(unsigned int freq, LP_Async_BWType bwType, double rxPowerDbm, int VSAGPort);
...
/*!
 * \param[in] freqStart, Starting frquency
 * \param[in] freqStop, Stop frequency
 * \param[in] freqStep, Step size of frequency sweep
 ...
 * \param[in] sweepTime, Sweep time input if autoSweepTime is set to 0
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_NFC_FreqSweep();
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_NFC_Setup_InitiatorWaveformAnalysis( NFC_STANDARD standard, NFC_DATARATE dataRate, double loadModAtten = 0, NFC_MOD_DEPTH modDepth = AUTO );
 ...
 * \param[in] freqOffset, Sets the frequency offset for NFC transeiver output
 * \param[in] pulseLength, Sets length of the modulation pulse.If pulse length equals to 0, then the pulse length is governed by the protocol selection. System would set default pulse length by different standards:
